...
# рлс (просчитывает, идут ли до ПРО), зрдн (такой же алгоритм)
        ...
        if (( $(echo "$distance_to_target <= $RADIUS" | bc -l) == 1 )); then
            if grep -q "$target_id" "$FILE_STAGE1"; then
                if ! grep -q "$target_id" "$FILE_STAGE2"; then    
                    ...
                    send_to_command_post "$(date) ПРО$PRO_NUM Обнаружена цель $target_type с ID:$target_id X=$target_x Y=$target_y"
                    ...
                    if [ "$target_type" == "$BM" ]; then
                        ...
                        if [ $COUNT_SHOOTS -gt 0 ]; then
                            ...
                            send_to_command_post "$(date) ПРО$PRO_NUM выстрел по цели ID:$target_id X=$target_x Y=$target_y"
                        else 
                            send_to_command_post "$(date) ПРО$PRO_NUM выстрел невозможен по цели ID:$target_id X=$target_x Y=$target_y"
                        fi
                    fi
                fi
                ...
                ((COUNT_SHOOTS--))
                send_to_command_post "$(date) ПРО$PRO_NUM промах по цели ID:$target_id X=$target_x Y=$target_y"
                send_to_command_post "$(date) ПРО$PRO_NUM повторный выстрел по цели ID:$target_id X=$target_x Y=$target_y"
                echo "$target_id" > "$DIR_DESTROY/$target_id"
            else
                send_to_command_post "$(date) ПРО$PRO_NUM выстрел невозможен по цели ID:$target_id X=$target_x Y=$target_y"
            fi
